...
class Wallet {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            function filterUtxos(utxos, utxoThreshold) {
                return utxos.filter((utxo) => {
                    const value = new bignumber_js_1.BigNumber(utxo.value);
                    if (value.gt(new bignumber_js_1.BigNumber(25).times(1e6)) &&
    ...
    splitUTXOS(utxos, keyPair, feeRate, utxoMinValue, UTXO_MIN_VALUE //UTXO_MIN_VALUE
    ) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (balance.lte(new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8))) {
                return {
                    hex: "",
            ...
            var totalOutputs = balance.dividedToIntegerBy(new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8));
            if (totalOutputs.toNumber() > 100) {
                ...
                outputs = 101;
                validUTXOs = selectUTXOs(validUTXOs, feeRate, new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8).times(100).toNumber(), outputs);
            ...
            var value = new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8).times(outputs);
            ...
            if (balance.minus(fee).lt(value)) {
                outputs -= 1;
                value = value.minus(new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8));
            }
            // If after the substitution the value is less than the minimum utxo value no point for transaction
            if (value.lt(new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8))) {
                return {
                    hex: "",
            ...
            tx.addOutput(from, balance
                .minus(fee)
                .minus(new bignumber_js_1.BigNumber(outputs - 1).times(UTXO_MIN_VALUE).times(1e8))
                .toNumber());
            for (var i = 0; i < outputs - 1; i++) {
                tx.addOutput(from, new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8).toNumber());
            }
            ...
            //////////////////SUPPORT FUNCTIONS/////////////////////
            function filterUtxos(utxos, UTXO_MIN_VALUE) {
                return utxos.filter((utxo) => {
                    const value = new bignumber_js_1.BigNumber(utxo.value);
                    if (value.gt(new bignumber_js_1.BigNumber(4).times(1e6))) {
                        if (!value.eq(new bignumber_js_1.BigNumber(UTXO_MIN_VALUE).times(1e8))) {
                            return true;
                        }
                    }
                    return false;
    ...
    optimizeWalletUTXOS(utxoMinValue, utxoThreshold //UTXO_MIN_VALUE
    ) {
        return __awaiter(this, void 0, void 0, function* () {
            const utxos = yield this.getBitcoinjsUTXOs();
            const infoRes = yield this.insight.getBlockchainInfo();
            const feeRate = Math.ceil(infoRes.feeRate * 1e5);
            let txResponse = yield this.splitUTXOS(utxos, this.keyPair, feeRate, utxoMinValue || 100, utxoThreshold || 150);
